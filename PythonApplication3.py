...
class Block:
    ...
    def mineblock(self,difficult):
        start = 0
        while [v for v in self.hash[start:difficult]] != ['5' for v in range(start, difficult)]:
        # while [v for v in self.hash[start:difficult]] != ['0' for v in range(start, difficult)]:
            #print([self.hash[start:difficult]])
            self.nonce += 1
           # print(self.nonce)
...
class BlockChain:
    ...
    def genesis_block(self):
        ...
        return Block([first_transaction], int(time.time()),"初始模块")
    # 获得区块链最新的block
    ...
    def check_chain_validity(self):
        for i in range(1,len(self.chain)):
            ...
            if(current_block.previous_hash != previous_block.hash):
                return False
        return True
    def Blockchain_display(self):
        print("begin to print the transacions in the whole block chain:")
        for i in range(0,len(self.chain)):
            for j in range(0,len(self.chain[i].transactions)):
                self.chain[i].transactions[j].Transaction_display()
def time_s():
